...
typedef struct {
    ...
    // 0x00000400 [10]    : STOP_DET_IF_MASTER_ACTIVE (0): Master issues the STOP_DET interrupt...
    // 0x00000200 [9]     : RX_FIFO_FULL_HLD_CTRL (0): This bit controls whether DW_apb_i2c should hold...
    // 0x00000100 [8]     : TX_EMPTY_CTRL (0): This bit controls the generation of the TX_EMPTY...
    // 0x00000080 [7]     : STOP_DET_IFADDRESSED (0): In slave mode: - 1'b1:  issues the STOP_DET...
    ...
    // 0x00000010 [4]     : IC_10BITADDR_MASTER (0): Controls whether the DW_apb_i2c starts its...
    // 0x00000008 [3]     : IC_10BITADDR_SLAVE (0): When acting as a slave, this bit controls whether...
    // 0x00000006 [1-2]   : SPEED (0x2): These bits control at which speed the DW_apb_i2c operates; its...
    // 0x00000001 [0]     : MASTER_MODE (1): This bit controls whether the DW_apb_i2c master is enabled
    io_rw_32 con;

    _REG_(I2C_IC_TAR_OFFSET)
    // I2C Target Address Register
    // 0x00000800 [11]    : SPECIAL (0): This bit indicates whether software performs a Device-ID or...
    ...
    // I2C Rx/Tx Data Buffer and Command Register; this is the register the CPU writes to when filling...
    // 0x00000800 [11]    : FIRST_DATA_BYTE (0): Indicates the first data byte received after the...
    // 0x00000400 [10]    : RESTART (0): This bit controls whether a RESTART is issued before the byte...
    // 0x00000200 [9]     : STOP (0): This bit controls whether a STOP is issued after the byte is sent...
    // 0x00000100 [8]     : CMD (0): This bit controls whether a read or a write is performed
    // 0x000000ff [0-7]   : DAT (0): This register contains the data to be transmitted or received on...
    io_rw_32 data_cmd;

    _REG_(I2C_IC_SS_SCL_HCNT_OFFSET)
    // Standard Speed I2C Clock SCL High Count Register
    // 0x0000ffff [0-15]  : IC_SS_SCL_HCNT (0x28): This register must be set before any I2C bus...
    io_rw_32 ss_scl_hcnt;

    _REG_(I2C_IC_SS_SCL_LCNT_OFFSET)
    // Standard Speed I2C Clock SCL Low Count Register
    // 0x0000ffff [0-15]  : IC_SS_SCL_LCNT (0x2f): This register must be set before any I2C bus...
    io_rw_32 ss_scl_lcnt;

    _REG_(I2C_IC_FS_SCL_HCNT_OFFSET)
    // Fast Mode or Fast Mode Plus I2C Clock SCL High Count Register
    // 0x0000ffff [0-15]  : IC_FS_SCL_HCNT (0x6): This register must be set before any I2C bus...
    ...
    // 0x00001000 [12]    : RESTART_DET (0): Indicates whether a RESTART condition has occurred on the...
    // 0x00000800 [11]    : GEN_CALL (0): Set only when a General Call address is received and it is...
    // 0x00000400 [10]    : START_DET (0): Indicates whether a START or RESTART condition has occurred...
    // 0x00000200 [9]     : STOP_DET (0): Indicates whether a STOP condition has occurred on the I2C...
    // 0x00000100 [8]     : ACTIVITY (0): This bit captures DW_apb_i2c activity and stays set until it...
    // 0x00000080 [7]     : RX_DONE (0): When the DW_apb_i2c is acting as a slave-transmitter, this bit...
    // 0x00000040 [6]     : TX_ABRT (0): This bit indicates if DW_apb_i2c, as an I2C transmitter, is...
    // 0x00000020 [5]     : RD_REQ (0): This bit is set to 1 when DW_apb_i2c is acting as a slave and...
    // 0x00000010 [4]     : TX_EMPTY (0): The behavior of the TX_EMPTY interrupt status differs based...
    // 0x00000008 [3]     : TX_OVER (0): Set during transmit if the transmit buffer is filled to...
    // 0x00000004 [2]     : RX_FULL (0): Set when the receive buffer reaches or goes above the RX_TL...
    // 0x00000002 [1]     : RX_OVER (0): Set if the receive buffer is completely filled to...
    // 0x00000001 [0]     : RX_UNDER (0): Set if the processor attempts to read the receive buffer when...
    ...
    // 0x00000001 [0]     : CLR_RX_OVER (0): Read this register to clear the RX_OVER interrupt (bit 1)...
    io_ro_32 clr_rx_over;

    _REG_(I2C_IC_CLR_TX_OVER_OFFSET)
    // Clear TX_OVER Interrupt Register
    // 0x00000001 [0]     : CLR_TX_OVER (0): Read this register to clear the TX_OVER interrupt (bit 3)...
    io_ro_32 clr_tx_over;

    _REG_(I2C_IC_CLR_RD_REQ_OFFSET)
    // Clear RD_REQ Interrupt Register
    // 0x00000001 [0]     : CLR_RD_REQ (0): Read this register to clear the RD_REQ interrupt (bit 5) of...
    io_ro_32 clr_rd_req;

    _REG_(I2C_IC_CLR_TX_ABRT_OFFSET)
    // Clear TX_ABRT Interrupt Register
    // 0x00000001 [0]     : CLR_TX_ABRT (0): Read this register to clear the TX_ABRT interrupt (bit 6)...
    io_ro_32 clr_tx_abrt;

    _REG_(I2C_IC_CLR_RX_DONE_OFFSET)
    // Clear RX_DONE Interrupt Register
    // 0x00000001 [0]     : CLR_RX_DONE (0): Read this register to clear the RX_DONE interrupt (bit 7)...
    ...
    // I2C Transmit FIFO Level Register This register contains the number of valid data entries in the...
    // 0x0000001f [0-4]   : TXFLR (0): Transmit FIFO Level
    io_ro_32 txflr;

    _REG_(I2C_IC_RXFLR_OFFSET)
    // I2C Receive FIFO Level Register This register contains the number of valid data entries in the...
    // 0x0000001f [0-4]   : RXFLR (0): Receive FIFO Level
    io_ro_32 rxflr;

    _REG_(I2C_IC_SDA_HOLD_OFFSET)
    // I2C SDA Hold Time Length Register
    // 0x00ff0000 [16-23] : IC_SDA_RX_HOLD (0): Sets the required SDA hold time in units of ic_clk...
    // 0x0000ffff [0-15]  : IC_SDA_TX_HOLD (1): Sets the required SDA hold time in units of ic_clk...
    io_rw_32 sda_hold;

    _REG_(I2C_IC_TX_ABRT_SOURCE_OFFSET)
    // I2C Transmit Abort Source Register
    // 0xff800000 [23-31] : TX_FLUSH_CNT (0): This field indicates the number of Tx FIFO Data Commands...
    ...
    // 0x00004000 [14]    : ABRT_SLV_ARBLOST (0): This field indicates that a Slave has lost the bus...
    // 0x00002000 [13]    : ABRT_SLVFLUSH_TXFIFO (0): This field specifies that the Slave has received...
    // 0x00001000 [12]    : ARB_LOST (0): This field specifies that the Master has lost arbitration, or...
    ...
    // 0x00000200 [9]     : ABRT_SBYTE_NORSTRT (0): To clear Bit 9, the source of the...
    // 0x00000100 [8]     : ABRT_HS_NORSTRT (0): This field indicates that the restart is disabled...
    // 0x00000080 [7]     : ABRT_SBYTE_ACKDET (0): This field indicates that the Master has sent a...
    // 0x00000040 [6]     : ABRT_HS_ACKDET (0): This field indicates that the Master is in High Speed...
    // 0x00000020 [5]     : ABRT_GCALL_READ (0): This field indicates that DW_apb_i2c in the master...
    // 0x00000010 [4]     : ABRT_GCALL_NOACK (0): This field indicates that DW_apb_i2c in master mode...
