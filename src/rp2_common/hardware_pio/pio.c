...
void pio_sm_set_input_sync_bypass_with_mask_internal(PIO pio, uint32_t bypass_enables, uint32_t pin_mask) {
    check_pio_param(pio);
...
void pio_sm_set_input_sync_bypass_with_mask(PIO pio, uint32_t bypass_enables, uint32_t pin_mask) {
    ...
    pin_mask >>= gpio_base;
#endif
    pio_sm_set_input_sync_bypass_with_mask_internal(pio, bypass_enables, pin_mask);
}
#endif

void pio_sm_set_input_sync_bypass_with_mask64(PIO pio, uint64_t bypass_enables, uint64_t pin_mask) {
    ...
    pin_mask >>= gpio_base;
#endif
    pio_sm_set_input_sync_bypass_with_mask_internal(pio, (uint32_t)bypass_enables, (uint32_t)pin_mask);
}
