...
To reduce duplication in version number strings, this script parses
pico_sdk_version.cmake to produce a version header containing the SDK version
information.
"""
...
class RudimentaryCmakeParser:
    ...
    def _expand_str(self, contents):
        return _EXPANSION_REGEX.sub(
            lambda val: str(self._known_vars.get(val.group(1))), contents
        )
    ...
    def _check_for_condition(self, line):
        ...
        if maybe_cond:
            self._condition_stack.append(
                bool(self._known_vars.get(maybe_cond.group(1)))
            )
