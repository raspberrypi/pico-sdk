 ...
 * --------------------------------------------------------------
 * The RP2040/RP2350 FreeRTOS ports implement the pico-sdk lock_core wait/notify
 * primitives with a single event group, mapping spin lock number -> event bit in
 * prvGetEventGroupBit():
 *
 *     32-bit ticks: bit = spin_lock_num % 24
 *     16-bit ticks: bit = spin_lock_num & 0x7
 *
 * so two *different* lock_core instances can share one bit. The wait is:
 *
 *     spin_unlock( pxLock->spin_lock, ulSave );            <-- window opens here
 *     xEventGroupWaitBits( ..., xClearOnExit=pdTRUE, ... );
 *
 * and xEventGroupSetBits() unblocks every task currently waiting on the bit, then
 * *clears* it. So in principle: a notify for lock V that lands while task VICTIM is inside
 * that window (not yet on the event list) unblocks a task THIEF waiting on an aliased lock
 * T instead, and the bit is cleared. THIEF re-checks its own condition, finds it
 * unsatisfied and re-waits -- crucially without ever acquiring or releasing lock V, so it
 * never re-sets the bit. VICTIM then blocks with its permit already granted and nobody
 * left to notify it.
 *
 * The port's own comment in vPortLockInternalSpinUnlockWithWait() argues this cannot
 * happen, because "any intervening blocked lock ... will need to unlock it before we
 * proceed, which will set the event bit again". That holds for a thief contending on the
 * *same* lock, but arguably not for one on an aliased lock. This test tries to settle it.
 *
 * Method: VICTIM repeatedly does sem_acquire_blocking() on a semaphore whose permit is
 * granted from a timer IRQ at a swept delay, so that across iterations the release sweeps
 * across the window. THIEF sits permanently blocked on a second semaphore. A monitor task
 * spots a stalled iteration and rescues it, so the run continues and can be counted.
 *
 * Two passes: a CONTROL pass with THIEF on a non-aliased lock, then the ALIASED pass. The
 * control must show zero stalls; if it doesn't, the harness is at fault and the aliased
 * number means nothing.
 *
 * ON A NEGATIVE RESULT: the vulnerable window runs only from spin_unlock() to the
 * vTaskSuspendAll() inside xEventGroupWaitBits() -- after that the waiter is placed on the
 * event list before the scheduler resumes, so a later notify is delivered correctly. That
 * is a handful of instructions. Zero stalls therefore means "not reproduced; the window is
 * very narrow", NOT "proven impossible".
 */
