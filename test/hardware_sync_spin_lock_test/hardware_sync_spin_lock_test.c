...
//
// The RP2350 lock_core wait/notify primitives (see PICO_SYNC_RP2350_SPIN_LOCK_WORKAROUND
// in pico/lock_core.h) are built on the claim that taking and releasing a software spin
// lock sets the calling core's *own* event flag, so a following __wfe() returns
// immediately and a second __wfe() is needed to actually wait. That claim is asserted at
// compile time by PICO_SPIN_LOCK_UNLOCK_CAUSES_SEV but is never checked against the
// hardware. It is worth checking, because the Arm and RISC-V software spin lock
// implementations do not use the same instructions -- ldaexb/strexb (which takes a
// reservation) vs amoor.w.aq (which does not) -- so it does not follow that both engage
// whatever generates the event.
//
// Measure it directly: clear the event flag, do a lock/unlock pair, then __wfe() and see
// whether it returned of its own accord or only once a backstop event arrived. Core 1
// provides the backstop (it spins for a while, then sets a flag and SEVs), so the __wfe()
// can never hang, and no timer is needed.
//
//   returned before the backstop => the lock/unlock set our event flag
//   returned only at the backstop => it did not
//
// A control pass with the lock/unlock omitted runs first. It must block, otherwise
// ambient interrupt activity is waking the __wfe() and the measurement means nothing; in
// that case the result is reported as inconclusive rather than as a failure.
//
// Only core 0 is probed. The event register is per core, but the two cores run the same
// spin lock code against the same fabric, so one core answers the question.
// ---------------------------------------------------------------------------
...
// Long enough that core 0 is certainly parked in its __wfe() before this expires; the
// exact duration doesn't matter, only that it is much longer than a FIFO push plus a
