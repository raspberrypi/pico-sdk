...
for gcc_version, toolchain_path in gcc_versions_sorted.items():
    for build_type in ["Debug", "Release"]:
        ...
        output += "      if: always()\n"
        output += "      shell: bash\n"
        output += "      run: cd ${{{{github.workspace}}}}; mkdir -p build; rm -rf build/*; cd build; cmake ../ -DPICO_SDK_TESTS_ENABLED=1 -DCMAKE_BUILD_TYPE={} -DPICO_TOOLCHAIN_PATH={} -DPICO_BOARD=pico_w; make -OrRj$(nproc)\n".format(build_type, toolchain_path)
